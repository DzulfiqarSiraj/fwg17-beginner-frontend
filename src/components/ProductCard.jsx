...
const ProductCard = ({id, image, name, description, basePrice, discount, isBestSeller}) => {
        ...
        <div className='flex flex-col items-center flex-1 relative'>
            ...
            <div className='bg-white shadow-md rounded-b p-3 mx-3 -mt-10 flex flex-col gap-4 flex-1'>
                ...
                <div className="flex flex-col md:flex-row md:gap-2 md:items-center">
                    {Number(discount) !== 0 ? <span className='text-[0.6rem] md:text-xs font-bold text-red-500'><del>Rp {Number(basePrice).toLocaleString('id')},-</del></span> : <span className='hidden'><del>Rp {Number(basePrice).toLocaleString('id')},-</del></span>}
                    
                    <span className='text-sm md:text-xl font-bold text-orange-500'>Rp {(Number(basePrice) - (Number(basePrice)*Number(discount))).toLocaleString('id')},-</span>
                </div>
                <div className='flex flex-row gap-2'>
                    <Link to={`${id}`} className="flex-1"><Button className='bg-orange-500 w-full py-1 md:py-2 text-[0.60rem] md:text-sm'>Buy</Button></Link>
